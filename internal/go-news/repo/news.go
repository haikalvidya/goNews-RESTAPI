...
func (n News) Get(id int) (*models.News, error) {
	// news := &models.News{}
	if err := clientDb.DbClient.Preload("Topic").First(&n, id).Error; err != nil {
		return nil, err
	}
	news := models.News(n)
	return &news, nil
...
func (n News) GetAll() ([]*models.News, error) {
	var manyNews []*models.News
	if err := clientDb.DbClient.Preload("Topic").Find(&manyNews).Error; err != nil {
		return nil, err
	}
	return manyNews, nil
}
